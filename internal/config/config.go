...
/* funtion to read config file. Looks for file at specific location specified in the os.Open call */
func Read() (Config, error) {
    configPath, err := getConfigFilePath()
    if err != nil {
        return Config{}, fmt.Errorf("Failed to retrieve config path with error: %v", err)
    }
    jsonFile, err := os.Open(configPath)
    if err != nil {
        return Config{}, fmt.Errorf("Failed to read with error: %v", err)
    }
    defer jsonFile.Close()
    byteContents, err := io.ReadAll(jsonFile)
    if err != nil {
        return Config{}, fmt.Errorf("Failed to read config file with error: %v", err)
    }
    var config Config
    err = json.Unmarshal(byteContents, &config)
    if err != nil {
        return Config{}, fmt.Errorf("Failed to unmarshal byte content to config struct with error: %v", err)
    }
    return config, nil
}
